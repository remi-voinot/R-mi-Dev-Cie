...
$commands_availaibles = array("air-msg" => "Air Message");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  if (isset($_GET) && isset($_GET["cmd"])) {
    if (!empty($_GET["cmd"])) {
      if (!empty($commands_availaibles[$_GET["cmd"]])) {
        $titlle = $commands_availaibles[$_GET["cmd"]];
        $m = true;
      } else {
        flashvar()->warning("La commande entrée en paramètre via l'URL n'existe pas, vous avez été redirigé sur la liste des commandes !");
        $titlle = "Liste des commandes premium";
        $m = false;
      }
    } else {
      $titlle = "Liste des commandes premium";
              ...
              <div class="card shadow mb-4 raw">
                <div class="card-header py-3 bg-gray-300 col-12">
                  <span class="m-0 font-weight-bold text-primary">
                    <?php
                    if ($m) {
                      echo ($commands_availaibles[$_GET["cmd"]]);
                    } else {
                      echo ("Liste des commandes premium");
                    }
                    ?>
                  </span>
                  <?php if ($m) echo ('
                  ...
                  <?php if(!$m) {
                    ...
                    <?php if($m) {
                      ...
                      <div class="input-group input-group-default col-12 col-md-4 sm-left-rounded-input sm-no-pad">
                        <span class="input-disabled-div">'.$commands_availaibles[$_GET["cmd"]].'</span>
                      </div>
